...
export function RenderCellIsReady({ params }: ParamsProps) {
  return (
    <Label
      variant="soft"
      color={(params.row.IsReady === true && 'info') || 'default'}
    >
      {params.row.IsReady === true ? 'Active' : 'Waiting...'}
    </Label>
  )
}
...
export function RenderCellStatus({ params }: ParamsProps) {
  return (
    <Label
      variant="soft"
      color={(params.row.Phase === 'Running' && 'success') || 'default'}
    >
      {params.row.Phase}
    </Label>
  )
...
export function RenderCellNsName({ params }: ParamsProps) {
  return (
    <Stack direction="row" alignItems="center" sx={{ py: 2, width: 1 }}>
      <ListItemText
        ...
        secondary={
          <Box
            component="div"
            sx={{ typography: 'body2', color: 'warning.main' }}
          >
            {params.row.Message}
          </Box>
        }
